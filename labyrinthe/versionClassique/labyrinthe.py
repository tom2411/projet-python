...
def getCoordonneesJoueurCourant(labyrinthe):
    ...
    #     lig+=1
    #print(joueur)
    for i in range(getNbLignes(labyrinthe['plateau'][0])):
        for j in range(getNbColonnes(labyrinthe['plateau'][0])):
            if len(labyrinthe['plateau'][0][i][j]['pion'])!=0:
                #print(labyrinthe['plateau'][0][i][j]['pion'][0])
                if joueur in labyrinthe['plateau'][0][i][j]['pion']:
                    return (j,i)
